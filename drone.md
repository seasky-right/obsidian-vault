...
- 冻结原来的airsim函数部分
- 封装新接口，便于以后调用不同底座（包括Airsim、Issac）
- 定义基础数据结构如 `TaskSpec` `Observation` `Action` `EpisodeResult`
...
- 把任务写成固定模版的可扩展模块，用户可以自定义任务
- 用一个`YAML`就能定义完整任务流程

**最终验收要求**：实现最基本的`Agent`底座，可以在不修改平台代码的情况下运行不同agent
